...
const FiltersBar = ({
  ...
  return (
    <div className="bg-[var(--color-bg-primary)] p-4 rounded-lg shadow-md grid grid-cols-1 md:grid-cols-3 gap-4">
      {/* Recherche */}
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 text-[var(--color-text-primary)] border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
        />
      </div>
      ...
      <div>
        ...
        >
          ...
          {uniqueSubjects.map((subject) => (
            <option key={subject.id_matiere} value={subject.id_matiere}>
              {subject.nom_matiere}
            </option>
          ))}
        </select>
