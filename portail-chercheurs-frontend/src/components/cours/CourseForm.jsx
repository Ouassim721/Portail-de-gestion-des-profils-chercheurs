...
function CourseForm() {
  ...
  const fetchMatieres = async () => {
    try {
      ...
      console.error(t("errorFetchingSubjects"), e);
    }
  };
  ...
  const fetchCours = async () => {
    try {
      ...
      console.error(t("errorFetchingCourse"), e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const validate = () => {
    ...
    if (formData.fichier) {
      if (formData.fichier.type !== "application/pdf") ne.fichier = t("errorFileType");
      if (formData.fichier.size > 10 * 1024 * 1024) ne.fichier = t("errorFileSize");
    }
    setErrors(ne);
  ...
  try {
    ...
    navigate(`/chercheurs/${id}/cours`);
      ...
      if (err.response?.status===422) {
        const se = err.response.data.errors, fe={};
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Titre */}
        <div>
          <label className="block text-gray-700 mb-2">{t("labelTitle")} *</label>
          <input
            type="text"
            name="titre"
        ...
        {/* Description */}
        <div>
          <label className="block text-gray-700 mb-2">{t("labelDescription")} *</label>
          <textarea
            name="description"
        ...
        {/* Matière */}
        <div>
          <label className="block text-gray-700 mb-2">{t("labelSubject")} *</label>
          <select
            name="id_matiere"
        ...
        <div>
          ...
          {isEdit && existingFile && (
            <div className="mb-3 p-3 bg-gray-50 rounded-lg">
              <p className="text-gray-600">{t("currentFile", { name: existingFile })}</p>
            </div>
          )}
          <input
            type="file"
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {loading
              ...
              ) : ( isEdit ? t("updateCourse") : t("createCourse") )
            }
          </button>
        </div>
      </form>
