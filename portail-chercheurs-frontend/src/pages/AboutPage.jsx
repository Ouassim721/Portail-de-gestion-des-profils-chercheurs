...
import placeholder from "../assets/blankpdp.png";
import {
  BookOpenIcon,
  ShieldCheckIcon,
  DocumentTextIcon,
...
const iconsMap = {
  frontend: (
    <CodeBracketIcon className="h-12 w-12 text-[var(--color-primary)] mb-4" />
  ),
  backend: (
    <CommandLineIcon className="h-12 w-12 text-[var(--color-primary)] mb-4" />
  ),
  database: (
    <CloudIcon className="h-12 w-12 text-[var(--color-primary)] mb-4" />
  ),
};
    ...
    <div className="min-h-screen bg-[var(--color-bg-secondary)]">
      ...
      <div className="bg-[var(--color-primary)] text-[var(--color-white)] py-20">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="text-xl text-[var(--color-gray)] max-w-3xl">
            {t("aboutSubtitle")}
          </p>
        </div>
      </div>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {t("teamMembers").map((m, idx) => (
              <div
                key={idx}
                className="bg-[var(--color-bg-primary)] p-6 rounded-xl shadow-md text-center"
              >
                <img
                  src={placeholder}
