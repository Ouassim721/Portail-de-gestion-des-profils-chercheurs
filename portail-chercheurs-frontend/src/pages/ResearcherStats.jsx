...
const ResearcherDashboard = () => {
    ...
    const fetchResearcherData = async () => {
      try {
        const [profileRes, pubsRes, followersRes, statsRes] = await Promise.all([
          ...
          axios.get("http://localhost:8000/api/chercheurs/followers", { withCredentials: true }),
          axios.get("http://localhost:8000/api/chercheurs/stats-personnelles", { withCredentials: true }),
        ]);
 ...
 setProfileData(profileRes.data);
  ...
  if (loading) return <div className="text-center py-8">Chargement de vos statistiques...</div>;

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Votre tableau de bord</h1>
        <p className="text-gray-600 mt-2">{profileData.nom} {profileData.prenom}</p>
      </div>
...
const ActivityItem = ({ title, value, date }) => (
  <div className="border-l-4 border-blue-500 pl-4 py-2">
    ...
    {date && <p className="text-sm text-gray-500 mt-1">{new Date(date).toLocaleDateString()}</p>}
  </div>
);
