      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isModalOpen && selectedCourse && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl overflow-auto max-h-[90vh] w-full max-w-4xl">
              <div className="p-6">
                ...
                <span className="inline-block px-3 py-1 text-xs font-semibold bg-blue-100 text-blue-700 rounded-full mb-4">
                  {selectedCourse.matiere?.nom_matiere || t('subjectUnspecified')}
                </span>
                ...
                {selectedCourse.fichier ? (
                  <>
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-lg font-medium">{t('attachedFile')}</h3>
                      <a
                        href={`${api.defaults.baseURL.replace('/api','')}/storage/${selectedCourse.fichier}`}
                        download
                    ...
                    <div className="h-[500px] border rounded overflow-hidden">
                      <iframe
                        src={`${api.defaults.baseURL.replace('/api','')}/storage/${selectedCourse.fichier}`}
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">{t('allCoursesTitle')}</h1>
          <p className="text-lg text-gray-600">{t('allCoursesSubtitle')}</p>
        </div>
          ...
          <div className="mt-4 text-sm text-gray-600 flex justify-between">
            <div>
              ...
              {(filter || selectedSubject) && (
                <>{t('coursesFoundWithTotal', { found: filteredCourses.length, total: courses.length })}</>
              )}
            </div>
            {(filter || selectedSubject) && (
              <button
                onClick={() => { setFilter(''); setSelectedSubject(''); }}
        ...
        {filteredCourses.length === 0 ? (
          <div className="bg-white p-8 rounded-xl shadow-lg text-center">
            ...
            <h3 className="text-xl font-medium mt-4">
              {courses.length === 0 ? t('noCoursesAvailable') : t('noCoursesMatch')}
            </h3>
            <p className="mt-2 text-gray-600">
              {courses.length === 0 ? t('noCoursesYet') : t('tryChangingFilters')}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCourses.map(course => (
              <div key={course.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition">
                <div className="p-6">
                  <div className="flex justify-between mb-4">
                  ...
                  <button
                    ...
                    className="text-left w-full font-bold text-gray-900 text-xl mb-2 hover:text-blue-600"
                  >
                    {course.titre}
                  </button>
                  ...
                  <div className="flex space-x-4 text-sm text-gray-500">
                    <div className="flex items-center">
                      ...
                      {t('instructorByLabel', { first: course.chercheur?.prenom, last: course.chercheur?.nom })}
                    </div>
                    <div className="flex items-center">
                      <CalendarIcon className="h-4 w-4 mr-1" />
                      {t('publishedOnLabel', { date: formatDate(course.datePublication) })}
                    </div>
                  </div>
                </div>
              </div>
